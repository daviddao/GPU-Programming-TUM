...
function [Transform, C]=cpd_register_float(X, Y, opt)
...
[C, W, sigma2, iter, T] = cpd_GRBF_float(single(X), single(Y), single(opt.beta), single(opt.lambda), single(opt.max_it), single(opt.tol), single(opt.outliers), single(opt.sigma2));

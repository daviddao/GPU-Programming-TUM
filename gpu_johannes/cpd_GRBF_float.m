...
G = cpd_G_mex(double(Y'),double(Y'),beta);
G = G+G';
...
%    [P1,Pt1, PX, L]=cpd_P(X,T, sigma2 ,outliers); st='';
    ...
    [P1,Pt1, PX, L]= cpd_P_GPU_float(X,T, sigma2 ,outliers); st='';
...
C = cpd_Pcorrespondence_float(X,T,sigma2save,outliers); 

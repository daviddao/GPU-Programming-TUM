...
__global__ void updatestepKernel(float *d_original, float * d_imgIn,
	...
	if (x < w && y < h && z < nc) {
		...
		float original = d_original[index];
		float updated = (2*original + lambda * (g_r * d_imgIn[index_xplus1] +
			g_l * d_imgIn[index_xminus1] + g_u * d_imgIn[index_yplus1] + g_d * d_imgIn[index_yminus1])) / 
					(2 + lambda * (g_r + g_l + g_u + g_d));
		updated = updated + theta * (updated - original);
...
//	create array filled with 1s
	...
	cudaMalloc(&d_floatarray, imgSize * sizeof(float));
	cudaMalloc(&d_arraysum, imgSize * sizeof(float));
	...
	for (int r = 0; r < repeats; r++) {
		...
		cudaMemcpy(d_floatarray, floatarray, imgSize * sizeof(float),
				cudaMemcpyHostToDevice);
		...
		arraysumkernel  <<<grid,block>>> (d_floatarray, d_arraysum, arraysize);
		cudaMemcpy(arraysum, d_arraysum, 1 * sizeof(float),
				cudaMemcpyDeviceToHost);
		timer.end();
		t = timer.get();  // elapsed time in seconds
		cout << "time GPU arraysum: " << t * 1000 << " ms" << endl;
...
//			divkernel <<<grid,block>>> (d_imgOut, d_imgOut2, d_imgOut3, w, h, nc);
		...
		updatestepKernel <<<grid,block>>> (d_original, d_imgIn, d_diffusity, d_imgOut, w, h, nc, tau, lambda, theta);
		CUDA_CHECK;

...
__global__ void gradientskernel(float *d_imgIn, float *d_imgOutX, float *d_imgOutY, int w, int h, int nc){
	...
	if (x<w && y < h && z < nc)
	{
		...
		if (x+1<w){
			...
			d_imgOutX[index] = d_imgIn[index2] - d_imgIn[index];
		}else{
			d_imgOutX[index]=0;
		}
		...
		if (y+1<h){
			...
			d_imgOutY[index] = d_imgIn[index3] - d_imgIn[index];
		}else{
			d_imgOutY[index]=0;
		}
	}
...
__global__ void updatestepKernel (float *d_imgIn, float *d_imgUpdateTerm, int w, int h, int nc, float tau)
{
	int x = threadIdx.x + blockDim.x * blockIdx.x;
	int y = threadIdx.y + blockDim.y * blockIdx.y;
	int z = threadIdx.z + blockDim.z * blockIdx.z;
	...
	{
		int index = x + y * w + z * w * h;
		float updated = d_imgIn[index];
		updated = updated + tau * d_imgUpdateTerm[index];
...
int main(int argc, char **argv) {
	...
	for (int r = 0; r < repeats; r++) {
		...
		for (int iteration = 0; iteration < N; iteration++){
			gradientskernel <<<grid,block>>> (d_imgIn, d_imgOut, d_imgOut2, w, h, nc);
			...
			//perform update step
			//tau = 0.2f/gHat;
			updatestepKernel <<<grid,block>>> (d_imgIn, d_imgOut3, w, h, nc, tau);
		}
